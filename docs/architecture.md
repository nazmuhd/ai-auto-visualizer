...
*   **Web Workers**: Heavy computational tasks are offloaded to background threads to keep the main thread responsive (60fps).
    ...
    *   `calculator.worker.ts`: Sorting, filtering, aggregating, and formula evaluation.
...
7.  **Transformation**: User actions in Data Studio trigger `calculator.worker.ts` to compute a new dataset view without freezing the UI.
