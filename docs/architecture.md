...
*   **Web Workers**: Heavy computational tasks are offloaded to background threads to keep the main thread responsive (60fps).
    ...
    *   `calculator.worker.ts`: Sorting, filtering, aggregating, and **secure formula evaluation**.
        *   *Security Note*: Formula evaluation uses a custom **Reverse Polish Notation (RPN)** tokenizer and parser. It strictly prohibits `eval()` or `new Function()`, preventing Arbitrary Code Execution (ACE) vulnerabilities from malicious user inputs.
