...
import { ReportTemplate, PresentationFormat } from '../../types.ts';
...
const PDF_TEMPLATES: ReportTemplate[] = [
  { id: 'mckinsey-exec-pdf', format: 'document', name: 'Executive Report', company: 'McKinsey/BCG', range: '3 Pages', description: 'Concise summary, top charts, and recommendations.' },
  { id: 'bain-ops-pdf', format: 'document', name: 'Operational Report', company: 'Bain/Deloitte', range: '5 Pages', description: 'KPIs, risks, and a clear action plan.' },
  { id: 'bcg-strat-pdf', format: 'document', name: 'Strategic Report', company: 'BCG/Accenture', range: '7-10 Pages', description: 'Deep market analysis and implementation roadmap.' },
  { id: 'pwc-comp-pdf', format: 'document', name: 'Compliance Report', company: 'PwC/EY', range: '20+ Pages', description: 'Detailed documentation with heavy text and appendices.' },
];
...
interface Props {
  ...
  preselectedFormat?: PresentationFormat | null;
}
...
export const ReportTemplateSelectionModal: React.FC<Props> = ({ isOpen, onClose, onSelect, preselectedFormat }) => {
  ...
  const [selectedFormat, setSelectedFormat] = useState<PresentationFormat | null>(null);
  ...
  const handleFormatSelect = (format: PresentationFormat) => {
      ...
      >
        <div className="p-6 pb-4 border-b border-slate-200 flex justify-between items-center flex-shrink-0">
          <h3 id="template-modal-title" className="text-xl font-bold text-slate-900">
            {step === 'format' ? 'Choose a Report Format' : `Select a ${selectedFormat === 'slides' ? 'Slide' : 'Document'} Template`}
          </h3>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          {step === 'format' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
              ...
              <button onClick={() => handleFormatSelect('document')} className="p-8 border-2 border-slate-200 rounded-xl flex flex-col items-center justify-center text-center hover:border-primary-500 hover:bg-primary-50/50 transition-all transform hover:-translate-y-1">
                <FileText size={64} className="text-primary-600 mb-4" />
                <h4 className="text-2xl font-bold text-slate-900">Document</h4>
