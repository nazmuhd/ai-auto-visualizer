...
export const DataStudio: React.FC<Props> = ({ project, onProjectUpdate }) => {
    ...
    const [isChooseColumnsModalOpen, setChooseColumnsModalOpen] = useState(false);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => { if (menuRef.current && !menuRef.current.contains(event.target as Node)) setOpenMenu(null); };
    ...
    return (
        <>
            <div className="flex h-full w-full bg-slate-100/50">
                <aside className={`flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm transition-all duration-300 ease-in-out ${isLeftPanelOpen ? 'w-64' : 'w-0 opacity-0'}`}>
                    ...
                    <div className="flex-shrink-0 p-3 space-y-2 border-b border-slate-100">
                        ...
                        <button onClick={() => setChooseColumnsModalOpen(true)} disabled={isGrouped} className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-md disabled:opacity-50 disabled:cursor-not-allowed"><Columns size={14} className="mr-2"/> Choose Columns</button>
                        ...
                        <div className="flex-1 overflow-auto custom-scrollbar">
                            <table className="min-w-full text-sm text-left">
                                <thead className="bg-slate-50 sticky top-0 z-10">
                                    <tr>{visibleHeaders.map(h => (<th key={h} className="px-4 py-2.5 font-medium text-slate-600 uppercase tracking-wider text-xs whitespace-nowrap group"><div className="flex items-center justify-between"><button onClick={() => handleSort(h)} className="flex items-center w-full text-left">{h}{sortConfig?.key === h && (sortConfig.direction === 'asc' ? <ArrowUp size={14} className="ml-2" /> : <ArrowDown size={14} className="ml-2" />)}{filteredColumnsSet.has(h) && <FilterIcon size={12} className="ml-2 text-primary-600" />}</button><div className="relative" ref={menuRef}><button onClick={() => setOpenMenu(openMenu === h ? null : h)} className="ml-2 p-1 rounded-md opacity-0 group-hover:opacity-100 focus:opacity-100 hover:bg-slate-200"><MoreVertical size={14} /></button>{openMenu === h && (<div className="absolute top-full right-0 mt-1.5 bg-white rounded-lg shadow-lg border border-slate-200 z-20 w-56 py-1">
                                        {columnTypes[h] === 'text' && <>
                                            <button onClick={() => handleSort(h)} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><ArrowUp size={14} className="mr-2"/> Sort A → Z</button>
                                            <button onClick={() => { addTransformation({ type: 'sort', payload: { key: h, direction: 'desc' } }); setOpenMenu(null); }} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><ArrowDown size={14} className="mr-2"/> Sort Z → A</button>
                                            <div className="my-1 border-t border-slate-100"></div>
                                            <div className="relative group/sub"><button className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center justify-between"><div className="flex items-center"><CaseSensitive size={14} className="mr-2"/> Transform</div><ChevronRight size={14} /></button><div className="absolute left-full -top-1 w-40 bg-white rounded-lg shadow-lg border border-slate-200 py-1 hidden group-hover/sub:block"><button onClick={() => handleTextTransform(h, 'uppercase')} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">UPPERCASE</button><button onClick={() => handleTextTransform(h, 'lowercase')} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">lowercase</button><button onClick={() => handleTextTransform(h, 'capitalize')} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">Capitalize</button></div></div>
                                        </>}
                                        {columnTypes[h] === 'number' && <>
                                            <button onClick={() => handleSort(h)} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><ArrowDown01 size={14} className="mr-2"/> Sort Smallest to Largest</button>
                                            <button onClick={() => { addTransformation({ type: 'sort', payload: { key: h, direction: 'desc' } }); setOpenMenu(null); }} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><ArrowUp10 size={14} className="mr-2"/> Sort Largest to Smallest</button>
                                            <div className="my-1 border-t border-slate-100"></div>
                                            <div className="relative group/sub"><button className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center justify-between"><div className="flex items-center"><Sigma size={14} className="mr-2"/> Quick Calculation</div><ChevronRight size={14} /></button><div className="absolute left-full -top-1 w-40 bg-white rounded-lg shadow-lg border border-slate-200 py-1 hidden group-hover/sub:block"><button onClick={() => handleQuickCalc(h, 'sum')} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">Sum</button><button onClick={() => handleQuickCalc(h, 'average')} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">Average</button><button onClick={() => handleQuickCalc(h, 'count')} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">Count</button></div></div>
                                        </>}
                                        {columnTypes[h] === 'date' && <>
                                            <button onClick={() => handleSort(h)} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><CalendarDays size={14} className="mr-2"/> Sort Oldest to Newest</button>
                                            <button onClick={() => { addTransformation({ type: 'sort', payload: { key: h, direction: 'desc' } }); setOpenMenu(null); }} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><CalendarDays size={14} className="mr-2"/> Sort Newest to Oldest</button>
                                        </>}
                                        <div className="my-1 border-t border-slate-100"></div>
                                        <button onClick={() => { setFilterModalOpen(true); setOpenMenu(null); }} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><FilterIcon size={14} className="mr-2"/> Filter...</button>
                                        <button onClick={() => handleHideColumn(h)} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><EyeOff size={14} className="mr-2"/> Hide Column</button>
                                        <button onClick={() => handleRenameColumn(h)} className="w-full text-left px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 flex items-center"><Pencil size={14} className="mr-2"/> Rename...</button>
                                    </div>)}</div></div></th>))}</tr>
                                </thead>
            ...
            {isChooseColumnsModalOpen && <ChooseColumnsModal allColumns={columnsAfterTransform} hiddenColumns={hiddenColumnsSet} onClose={() => setChooseColumnsModalOpen(false)} onApply={(colsToHide) => addTransformation({ type: 'hide_columns', payload: { columns: colsToHide } })} />}
